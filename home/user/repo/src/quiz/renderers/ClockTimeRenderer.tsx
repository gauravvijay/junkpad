...
const ClockTimeRenderer: React.FC<QuestionRendererProps> = ({
  ...
  const { hour, minutes } = question.data as {
    hour: number;
    minutes: number;
  };
  ...
  const hourAngle = (hour % 12) * 30 + minutes * 0.5; // 360/12 = 30 deg per hour + minute offset
    ...
    <div className={styles.questionContainer}>
      ...
      <div className={styles.timeInputRow}>
        <input
          ...
          disabled={!!locked}
          className={`${styles.numberInput} ${styles.timeInput}`}
          placeholder="H"
          min={1}
          max={12}
        />
