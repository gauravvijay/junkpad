...
const PlaceValueRenderer: React.FC<QuestionRendererProps> = ({
  ...
  const [assignments, setAssignments] = useState<Record<string, number | null>>(
    () => {
      ...
      matchItems.forEach((_: MatchItem, idx: number) => {
        init[`item_${idx}`] = null;
      });
      return init;
    }
  ...
  return (
    <div className={styles.questionContainer}>
      ...
      <div className={styles.matchGrid}>
        {matchItems.map((item: MatchItem, idx: number) => {
          const key = `item_${idx}`;
          const isCorrect =
            locked &&
            (question.correctAnswer as Record<string, number>)[key] === assignments[key];
      ...
      {!locked && (
        <button onClick={handleSubmit} disabled={!allAssigned} className={styles.submitBtn}>
          Submit
        </button>
      )}
    </div>
