...
const GraphApplesRenderer: React.FC<QuestionRendererProps> = ({
  ...
  const { bars, askChildren, totalApples: _totalApples } = question.data as {
    bars: BarData[]; askChildren: number; askApples: number; totalApples: number;
  };
  void _totalApples;

  const correctAnswer = question.correctAnswer as { perGroup: number; total: number };
  ...
  const handleSubmit = () => {
    onAnswer({ perGroup: Number(perGroupInput), total: Number(totalInput) });
  };
    ...
    <div className={styles.questionContainer}>
      <div className={styles.graphContainer}>
        ...
        <div className={styles.graphChart}>
          <div className={styles.graphYAxis}>
            {Array.from({ length: maxApples + 1 }).map((_, i) => maxApples - i).map((val) => (
              <div key={val} className={styles.graphYTick}>{val}</div>
            ))}
          </div>
              ...
              <div key={idx} className={styles.graphBarCol}>
                <div className={styles.graphBar}
                  style={{
                    ...
                    backgroundColor: bar.children === askChildren ? "#e91e63" : "#4CAF50",
                  }}>
      ...
      <div className={styles.graphQuestions}>
        <div className={styles.graphQuestion}>
          <label>
            How many apples were eaten by the group of <strong>{askChildren}</strong> children?{" "}
            <span style={{ color: "#e91e63" }}>(highlighted in pink)</span>
          </label>
          <input type="number" value={perGroupInput} onChange={(e) => setPerGroupInput(e.target.value)}
            disabled={!!locked}
        ...
        <div className={styles.graphQuestion}>
          <label>
            How many <strong>total</strong> apples were eaten by all groups?<br />
            <span className={styles.hint}>(Hint: multiply children x apples for each group, then add them up)</span>
          </label>
      ...
      {locked && (
        <div className={styles.answerReveal}>
          Correct: per group = <strong>{correctAnswer.perGroup}</strong>, total = <strong>{correctAnswer.total}</strong>
        </div>
      )}
      {!locked && (
        <button onClick={handleSubmit} disabled={perGroupInput === "" || totalInput === ""} className={styles.submitBtn}>
          Submit
        </button>
      )}
    </div>
