...
const SudokuRenderer: React.FC<QuestionRendererProps> = ({
  ...
  const { targetSum, puzzle, gaps } = question.data as {
    targetSum: number; puzzle: number[][]; gaps: [number, number][];
  };
  const solution = question.correctAnswer as number[][];

  const [inputs, setInputs] = useState<Record<string, string>>(() => {
    const init: Record<string, string> = {};
    gaps.forEach(([r, c]: [number, number]) => { init[`${r}_${c}`] = ""; });
    return init;
  });

  const locked = result?.answered;
  const isGap = (r: number, c: number) => gaps.some(([gr, gc]: [number, number]) => gr === r && gc === c);
  ...
  const handleSubmit = () => {
    const userGrid = puzzle.map((row: number[]) => [...row]);
    gaps.forEach(([r, c]: [number, number]) => {
  ...
  return (
    <div className={styles.questionContainer}>
      ...
      <table className={styles.sudokuGrid}>
        <tbody>
          {puzzle.map((row: number[], r: number) => (
            <tr key={r}>
              {row.map((cell: number, c: number) => {
                  ...
                  return (
                    <td key={c} className={cellClass}>
                      <input type="number" value={inputs[key]}
                        ...
                        disabled={!!locked} className={styles.sudokuInput} />
                ...
                return <td key={c} className={styles.sudokuCell}>{cell}</td>;
              })}
          ...
          <tr>
            {[0, 1, 2].map((c) => (
              <td key={c} className={styles.sudokuColSum}>{"\u2193"} {targetSum}</td>
            ))}
            <td />
          </tr>
        </tbody>
      ...
      {!locked && (
        <button onClick={handleSubmit} disabled={!allFilled} className={styles.submitBtn}>Submit</button>
      )}
    </div>
  );
};
